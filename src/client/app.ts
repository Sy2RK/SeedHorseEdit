...
const assetList = byId<HTMLDivElement>('assetList');
const inputStage = byId<HTMLDivElement>('inputStage');
const resultStage = byId<HTMLDivElement>('resultStage');
const jsonOut = byId<HTMLPreElement>('jsonOut');
const progressFill = byId<HTMLDivElement>('progressFill');
const progressText = byId<HTMLDivElement>('progressText');

const saved = JSON.parse(localStorage.getItem('seed-horse-settings') || '{}') as SavedSettings;
...
function setInputValue(id: string, value: string): void {
  byId<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(id).value = value;
}
...
function updateProviderFields(): void {
  document.querySelectorAll<HTMLElement>('label[data-provider]').forEach((el) => {
...
function iconFor(kind: MediaKind): string {
  if (kind === 'video') return 'clapperboard';
  if (kind === 'audio') return 'audio-lines';
  return 'image';
}

function setJson(payload: unknown): void {
  state.lastJson = payload || {};
  jsonOut.textContent = JSON.stringify(state.lastJson, null, 2);
}

function setProgress(message: string, level: 'info' | 'success' | 'error' = 'info', percent?: number): void {
...
function startPolling(interval = 10000): void {
  stopPolling();
...
function primaryVideo(): Asset | undefined {
  return state.assets.find((a) => a.kind === 'video' && a.primary);
}

function autoSetPrimary(): void {
...
function setPrimaryAsset(id: string): void {
  state.assets.forEach((a) => { a.primary = false; });
  const asset = state.assets.find((a) => a.id === id);
  if (asset) asset.primary = true;
  renderAssets();
  updateDurationDisplay();
}

function floorDuration(seconds: number, step = 5): number {
...
function normalizeAsset(file: AssetInput): Asset {
...
function addAssets(files: AssetInput[]): void {
...
function detectVideoDurations(): void {
...
function collectParameters(): Record<string, string | boolean | number | undefined> {
  const durationInput = inputValue('duration').trim();
  const pv = primaryVideo();
  let duration: string | number | undefined = durationInput || undefined;
  if (!duration && pv?.duration) {
    duration = floorDuration(pv.duration);
  }
  return {
    resolution: inputValue('resolution'),
    watermark: byId<HTMLInputElement>('watermark').checked,
...
function setTaskMeta(task: TaskState = {}): void {
  byId<HTMLElement>('metaProvider').textContent = task.provider || '-';
  byId<HTMLElement>('metaTask').textContent = task.taskId || '-';
  byId<HTMLElement>('metaStatus').textContent = task.status || '-';
}

function showStage(message: string, videoUrl?: string): void {
  if (videoUrl) {
...
async function healthCheck(): Promise<void> {
  try {
    ...
    byId<HTMLElement>('serverStatus').textContent = '已连接';
    byId<HTMLElement>('serverStatus').classList.add('ok');
  } catch {
    byId<HTMLElement>('serverStatus').textContent = '未连接';
    byId<HTMLElement>('serverStatus').classList.remove('ok');
  }
}
...
async function generate(): Promise<void> {
  ...
  setJson(payload);
  byId<HTMLButtonElement>('poll').disabled = !payload.taskId;
  showStage('任务已提交');
...
async function pollTask(): Promise<void> {
  if (!state.task?.provider || !state.task.taskId) return;
  persistSettings();
  showStage('查询中');
  setProgress('查询任务状态...');
  ...
  const payload = await parseJsonResponse<PollResponse>(response);
...
function wireEvents(): void {
  document.querySelectorAll<HTMLButtonElement>('.switcher button').forEach((button) => {
    button.addEventListener('click', () => {
      state.provider = button.dataset.provider as Provider;
      document.querySelectorAll<HTMLButtonElement>('.switcher button').forEach((item) => {
  ...
  byId<HTMLButtonElement>('addUrl').addEventListener('click', () => {
  ...
  assetList.addEventListener('click', (event) => {
    ...
    const primaryBtn = target.closest<HTMLButtonElement>('[data-set-primary]');
    if (primaryBtn) {
      setPrimaryAsset(primaryBtn.dataset.setPrimary || '');
    }
  });

  byId<HTMLButtonElement>('clearAssets').addEventListener('click', () => {
    state.assets = [];
    state.previewIndex = 0;
    renderAssets();
  });
  ...
  inputStage.addEventListener('click', (event) => {
    const tab = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-preview-index]');
    if (!tab) return;
  ...
  byId<HTMLButtonElement>('poll').addEventListener('click', async () => {
    try {
      await pollTask();
    } catch (error) {
  ...
  byId<HTMLButtonElement>('copyJson').addEventListener('click', async () => {
    await navigator.clipboard.writeText(JSON.stringify(state.lastJson, null, 2));
  });

  byId<HTMLButtonElement>('clearLog').addEventListener('click', resetProgress);

  ['aliKey', 'byteKey'].forEach((id) => {
    byId<HTMLInputElement>(id).addEventListener('change', persistSettings);
  });
}
